...
def main():
    try:
        ...
        # 学習データの平均価格を基準に正規化
        reference_price = train_data['Close'].mean()
        print(f"学習データの平均価格（正規化基準）: {reference_price:.2f}")
        
        # データの前処理
        train_data, features = prepare_data(train_data, reference_price)
        test_data, _ = prepare_data(test_data, reference_price)
