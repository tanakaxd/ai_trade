...
# 設定
TRAIN_MODEL = True  # True: モデルを学習
MODEL_VERSION = 'v2'  # モデルのバージョン管理用（変更を反映）

# Parameter grid
SEQUENCE_LENGTH = 100
LOOKAHEAD_PERIOD = 10
SCALING_FACTOR = 400  # 200 → 400
THRESHOLD = 0.1       # 0.2 → 0.1
MAX_POSITION = 2.0
TRANSACTION_COST = 0.0005  # 0.001 → 0.0005
ticker = "Nikkei225"
...
TEST_CSV = os.path.join(MASTER_DATA_DIR, '9432.T_20250303_20250430_5min.csv')
...
def normalize_price(data, reference_price):
    """価格データを正規化"""
    data = data.copy()
    data['Close'] = data['Close'] / reference_price
    data['High'] = data['High'] / reference_price
    data['Low'] = data['Low'] / reference_price
    return data

def prepare_data(data, reference_price=None):
    """データの前処理"""
    print("価格データを正規化中...")
    if reference_price is not None:
        data = normalize_price(data, reference_price)
...
def evaluate_model(model, data, scaler, features, is_test=False):
    ...
    predictions = predictions * 0.5  # 予測値をスケーリング（0.5倍）
    
    # 予測値をデータフレームに追加
    data['Predicted_Return'] = np.nan
    pred_df = pd.DataFrame(predictions.flatten(), index=indices, columns=['Predicted_Return'])
    data.update(pred_df)
...
def main():
    try:
        # データの読み込み（日経225）
        print("データを読み込み中...")
        train_data_nikkei = pd.read_csv(INPUT_CSV, parse_dates=['Datetime'])
        train_data_nikkei['Datetime'] = pd.to_datetime(train_data_nikkei['Datetime'], errors='coerce')
        train_data_nikkei = train_data_nikkei[(train_data_nikkei['Datetime'] >= TRAIN_START_DATE) & 
                                              (train_data_nikkei['Datetime'] <= TRAIN_END_DATE)]
        train_data_nikkei = train_data_nikkei.set_index('Datetime')
        
        # トレーニングデータに9432.Tを追加
        train_data_9432 = pd.read_csv(TEST_CSV, parse_dates=['Datetime'])
        train_data_9432['Datetime'] = pd.to_datetime(train_data_9432['Datetime'], errors='coerce')
        train_data_9432 = train_data_9432[(train_data_9432['Datetime'] >= TRAIN_START_DATE) & 
                                          (train_data_9432['Datetime'] <= TRAIN_END_DATE)]
        train_data_9432 = train_data_9432.set_index('Datetime')
        
        # データの結合
        train_data = pd.concat([train_data_nikkei, train_data_9432], axis=0)
        ...
        # 各データセットの平均価格を基準に正規化
        train_reference_price = train_data['Close'].mean()
        test_reference_price = test_data['Close'].mean()
        print(f"学習データの平均価格（正規化基準）: {train_reference_price:.2f}")
        print(f"テストデータの平均価格（正規化基準）: {test_reference_price:.2f}")
        
        # データの前処理
        train_data, features = prepare_data(train_data, train_reference_price)
        test_data, _ = prepare_data(test_data, test_reference_price)
